...
loyalty_avg_days_since_last_order AS (
  SELECT
    ...
    , ROUND(
        AVG(avg_days_since_last_order)
      , 2
      ) AS avg_days_since_last_order
    , COUNT(*) AS frequency 
  FROM
    customer_avg_days_since_last_order
...
FROM 
  overall_avg_days_since_last_order
UNION ALL
(SELECT
  loyalty_status
  , avg_days_since_last_order
FROM 
  loyalty_avg_days_since_last_order)
ORDER BY
  avg_days_since_last_order DESC;
